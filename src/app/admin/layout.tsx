...
import {
    LayoutDashboard, Package, Newspaper, FolderKanban,
    ShoppingCart, FileText, Inbox, Users,
    UserCircle, Menu, X, LogOut, Sparkles, Flame,
} from "lucide-react";
import { useAuth } from "@/shared/providers/AuthProvider";
import { Loading } from "@/shared/components/admin/ui";
import { canAccessAdminRoute, hasCapability } from "@/shared/auth/capabilities";
import { AccessDenied } from "@/shared/components/admin/PermissionGuard";
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
    ...
    const hasAccess = (item: NavItem) => {
        ...
        return hasCapability(user, item.permission);
    };
    ...
    const Sidebar = (
        <div className="flex h-full flex-col">
            ...
            <nav aria-label="Navegação administrativa" className="flex-1 overflow-y-auto px-3 py-4 space-y-5">
            ...
            <div className="p-3 border-t border-gray-100 dark:border-white/5 space-y-2">
                ...
                <button
                    ...
                    className="flex w-full items-center gap-3 rounded-xl px-3 py-2 text-sm font-medium text-red-500 hover:bg-red-500/10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary transition-colors"
                >
        ...
        <div className="min-h-screen bg-gray-50 dark:bg-[#0b1120]">
            ...
            {open && (
                <div className="fixed inset-0 z-50 lg:hidden">
                    <button aria-label="Fechar menu" className="absolute inset-0 bg-black/60" onClick={() => setOpen(false)} />
                    <aside className="absolute inset-y-0 left-0 w-72 bg-white dark:bg-[#0f172a] shadow-2xl animate-in slide-in-from-left duration-200">
                        <button aria-label="Fechar navegação" onClick={() => setOpen(false)} className="absolute top-4 right-4 p-1.5 rounded-lg text-gray-400 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:hover:bg-white/10">
                            <X className="w-5 h-5" />
                        </button>
                        {Sidebar}
                    </aside>
            ...
            <div className="lg:pl-64">
                <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-gray-100 dark:border-white/5 bg-white/80 dark:bg-[#0b1120]/80 backdrop-blur px-6 py-4 lg:hidden">
                    <button aria-label="Abrir navegação" onClick={() => setOpen(true)} className="p-1.5 rounded-lg text-gray-500 hover:bg-gray-100 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-primary dark:hover:bg-white/10">
                ...
                <main className="p-4 sm:p-6 lg:p-10 max-w-7xl">
                    {canAccessAdminRoute(user, pathname) ? children : (
