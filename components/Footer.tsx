        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
          <div className="col-span-1 md:col-span-2 lg:col-span-1">
            ...
            <div className="flex gap-4">
              {[
                { icon: 'instagram', url: 'https://www.instagram.com/vitaleevo/' },
                { icon: 'facebook', url: 'https://www.facebook.com/vitaleevo' },
                { icon: 'tiktok', url: 'https://www.tiktok.com/@vitaleevonegocios' }
              ].map((social) => (
                <a 
                  key={social.icon} 
                  href={social.url} 
                ...
                >
                  ...
                  {/* Fallback Material Icon if font-awesome is not loaded */}
                  {!social.icon.includes('fa-') && <span className="material-icons-round text-lg">
                    {social.icon === 'instagram' ? 'camera_alt' : social.icon === 'facebook' ? 'facebook' : 'audiotrack'}
                  </span>}
                </a>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-3 text-sm text-gray-600 dark:text-gray-400">
              <li><button onClick={() => onNavigate(View.Home)} className="hover:text-primary transition-colors">Home</button></li>
              <li><button onClick={() => onNavigate(View.About)} className="hover:text-primary transition-colors">Sobre Nós</button></li>
              <li><button onClick={() => onNavigate(View.Portfolio)} className="hover:text-primary transition-colors">Portfólio</button></li>
              <li><button onClick={() => onNavigate(View.Careers)} className="hover:text-primary transition-colors">Carreiras</button></li>
            </ul>
          </div>
          <div>
